...
struct PartitionType partition_types[] = {
    {0x00, "Empty"} ,
  	{0x01, "FAT12"},
	{0x02, "XENIX root"},
	{0x03, "XENIX usr"},
	...
	{0x83, "Linux"},
	{0x84, "OS/2 hidden or Intel hibernation"},/* OS/2 hidden C: drive,
	...
	{0xfd, "Linux raid autodetect"},/* Linux raid partition with
...
char* get_part_info(char p){
    char* tmp = "Unknown";
    for(int i = 0; i < ARRAY_SIZE(partition_types); i++){
        if(partition_types[i].type == p){
            tmp = partition_types[i].desc;
            break;
        }
    }
    return tmp;
